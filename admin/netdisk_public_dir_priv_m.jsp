...
while (ir.hasNext()) {
  ...
  <form id="form<%=i%>" name="form<%=i%>" action="?op=modify" method=post>
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
      <td style="PADDING-LEFT: 10px">&nbsp;<img src="../admin/images/arrow.gif" align="absmiddle">&nbsp;
	  <%
      ...
      <td><%=lp.getTypeDesc()%><!--<input name=see type=checkbox <%=lp.getSee()==1?"checked":""%> value="1">浏览&nbsp;
...
&nbsp;<input type=button onClick="window.location.href='netdisk_public_dir_priv_m.jsp?op=del&dirCode=<%=StrUtil.UrlEncode(leaf.getCode())%>&id=<%=lp.getId()%>'" value=删除> </td>
    </tr></form>
<%}%>
  </tbody>
</table>
...
<DIV style="WIDTH: 95%" align=right>
  <INPUT 
onclick="javascript:location.href='netdisk_public_dir_priv_add.jsp?dirCode=<%=StrUtil.UrlEncode(leafPriv.getDirCode())%>';" type=image 
