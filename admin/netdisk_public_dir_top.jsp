...
<table width='100%' cellpadding='0' cellspacing='0' >
  <tr>
    <td class="head">管理&nbsp;<%=root_name%>&nbsp;&nbsp;<a target="_parent" href="../netdisk/netdisk_public_share_frame.jsp">浏览视图</a></td>
  </tr>
</table>
<br>
...
border=0>
          <tbody>
            ...
            <td width="34%" align=right nowrap>>>&nbsp;<%=root_name%>&nbsp;&nbsp;<a target="_parent" href="document_list_m.jsp?dir_code=<%=StrUtil.UrlEncode(root_code)%>&dir_name=<%=StrUtil.UrlEncode(root_name)%>"></a>&nbsp;<a target=_parent href='netdisk_public_dir_priv_m.jsp?dirCode=<%=StrUtil.UrlEncode(root_code)%>'>权限</a>&nbsp;&nbsp;<a target=dirbottomFrame href="netdisk_public_dir_bottom.jsp?parent_code=<%=StrUtil.UrlEncode(root_code, "utf-8")%>&parent_name=<%=StrUtil.UrlEncode(root_name, "utf-8")%>&op=AddChild">添子目录</a> <a target="dirbottomFrame" href="netdisk_public_dir_bottom.jsp?op=modify&code=<%=StrUtil.UrlEncode(root_code, "utf-8")%>&name=<%=StrUtil.UrlEncode(root_name,"utf-8")%>&description=<%=StrUtil.UrlEncode(root_description,"utf-8")%>">修改</a> <!--<a target=_self href="#" onClick="if (window.confirm('您确定要删除<%=root_name%>吗?')) window.location.href='dir_top.jsp?op=del&delcode=<%=root_code%>'">删除</a>-->			  </td>
            </tr>
          </tbody>
        </table>
<%
