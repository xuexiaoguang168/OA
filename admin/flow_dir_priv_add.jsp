...
<table cellSpacing="0" cellPadding="0" width="100%">
  <tbody>
    <tr>
      <td class="head">设置目录 <a href="flow_dir_priv_m.jsp?dirCode=<%=StrUtil.UrlEncode(dirCode)%>"><%=leaf.getName()%></a> 权限</td>
    </tr>
  </tbody>
</table>
<br>
...
while (ir.hasNext()) {
    ...
    <tr class="row" style="BACKGROUND-COLOR: #ffffff">
      ...
      <td><%=desc%></td>
      <td>
	  <a href="flow_dir_priv_m.jsp?op=add&dirCode=<%=StrUtil.UrlEncode(leaf.getCode())%>&name=<%=StrUtil.UrlEncode(code)%>&type=<%=LeafPriv.TYPE_USERGROUP%>">[ 添加 ]</a></td>
    </tr>
<%}%>
  </tbody>
</table>
