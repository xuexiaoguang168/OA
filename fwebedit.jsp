		...
		if (isSucceed) {
			if (op=="edit")
			{
				...
				if (confirm("<%=correct_result%> 点击确定继续转至目录列表，点击取消继续添加！\r\n"))
					window.location.href = "cms/document_list_m.jsp?dir_code=<%=StrUtil.UrlEncode(dir_code)%>&dir_name=<%=StrUtil.UrlEncode(dir_name)%>";
				else
          ...
          <TABLE cellSpacing=0 cellPadding=0 width="100%">
            <TBODY>
              <TR>
				...
				<%
				if (op.equals("add")) {%>
					添加内容至--<a href="cms/document_list_m.jsp?dir_code=<%=StrUtil.UrlEncode(dir_code)%>&dir_name=<%=StrUtil.UrlEncode(dir_name)%>"><%=dir_name%></a>
				<%}else{%>
					修改--
					<%
					Leaf dlf = new Leaf();
...
<!--( <a href="cms/comment_m.jsp?doc_id=<%=doc.getID()%>">管理评论</a> )-->
<%}%></td>
            </tr>
            <tr>
...
%>			  </td>
            ...
            <tr>
              <td align="left" valign="middle" bgcolor="#F2F2F2" class="unnamed2">作&nbsp;&nbsp;&nbsp;&nbsp;者：</td>
              <td bgcolor="#F2F2F2">
			  <%
			  String userName = "";
			  ...
			  %>			  
			  <input name="author" id="author" type="TEXT" size=30 maxlength=100 style="background-color:ffffff;color:000000;border: 1 double" value="<%=userName%>">
            ...
            <tr bgcolor="#F2F2F2">
              ...
              <td bgcolor="#F2F2F2"><input name="keywords" id=keywords type="TEXT" size=30 maxlength=100 style="background-color:ffffff;color:000000;border: 1 double" value="<%=StrUtil.getNullStr(doc==null?dir_name:doc.getKeywords())%>">
              ( 请用&quot;，&quot;号分隔)
			<input type="hidden" name="isRelateShow" value="1">			  </td>
            </tr>
            <tr align="left">
...
--></td>
            ...
            <tr align="left" bgcolor="#F2F2F2">
			  ...
			  %>
                <input name="templateId" class="singleboarder" value="<%=templateId%>" size=3 type=hidden>
&nbsp;<a href="javascript:showModalDialog('cms/doc_template_select_frame.jsp',window.self,'dialogWidth:640px;dialogHeight:480px;status:no;help:no;')">选模板</a> <span id=templateInfo>
<%if (doc!=null && doc.getTemplateId()!=doc.NOTEMPLATE) {%>
<a target=_blank href="cms/doc_template_show.jsp?id=<%=doc.getTemplateId()%>">预览模板</a>
...
<%}else{%> 
<a href="#" onClick="if (addform.templateId.value!=-1) window.open('cms/doc_template_show.jsp?id=' + addform.templateId.value); else alert('当前未选择模板')">预览模板</a>
&nbsp;<a href="#" onClick="addform.templateId.value='-1';oEdit1.putHTML(' ')">取消模板</a>
<%}%>
</span></td>
            </tr>
            <tr align="left" bgcolor="#F2F2F2">
              <td valign="middle" bgcolor="#FFFFFF" class="unnamed2">颜&nbsp;&nbsp;&nbsp;&nbsp;色：</td>
              <td valign="middle" bgcolor="#FFFFFF" class="unnamed2"><select name="color">
...
标题加粗 
                  到期时间：
<input readonly type="text" id="expireDate" name="expireDate" size="10" onClick="showcalendar(event, this)" onFocus="showcalendar(event, this);if(this.value=='0000-00-00') this.value=''" value="<%=strExpireDate%>"></td>
            </tr>
            <tr bgcolor="#F2F2F2">
...
<%	}
	else if (action.equals("selTemplate")) {
%>	
		oEdit1.RENDER(document.getElementById("divTemplate").innerHTML);
<%	}
	else {%>
...
&nbsp;<a href="javascript:changeAttachName('<%=am.getId()%>', '<%=doc.getID()%>', '<%="attach_name"+am.getId()%>')">更改</a>                        &nbsp;<a href="javascript:delAttach('<%=am.getId()%>', '<%=doc.getID()%>')">删除</a>&nbsp;&nbsp;<a target=_blank href="<%=am.getVisualPath() + "/" + am.getDiskName()%>">查看</a>&nbsp;<a href="?op=edit&id=<%=doc.getID()%>&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&action=changeAttachOrders&direction=up&attachId=<%=am.getId()%>"><img src="images/arrow_up.gif" alt="往上" width="16" height="20" border="0" align="absmiddle"></a>&nbsp;<a href="?op=edit&id=<%=doc.getID()%>&dir_code=<%=StrUtil.UrlEncode(dir_code)%>&action=changeAttachOrders&direction=down&attachId=<%=am.getId()%>"><img src="images/arrow_down.gif" alt="往下" width="16" height="20" border="0" align="absmiddle"></a></td>
                      </tr>
                    </table>
				<%}
			  }
...
%><object classid="CLSID:DE757F80-F499-48D5-BF39-90BC8BA54D8C" codebase="<%=request.getContextPath()%>/activex/webedit.cab#version=4,0,1,1" width=400 height=173 align="middle" id="webedit">
                      ...
                      <param name="ForeColor" value="(155,155,230)">
            ...
            <tr>
			  ...
			  <div id=recorderDiv style="display:none"><OBJECT ID="Recorder" CLASSID="CLSID:E4A3D135-E189-48AF-B348-EF5DFFD99A67"></OBJECT></div></td>
            </tr>
            <tr>
			  ...
			  %>
<!--			  <%if (templateId==-1) {%>
              <input name="cmdok2" type="button" class="singleboarder" value="<%=action%>(续传)" onClick="return SubmitWithFileDdxc()">
              <%}
			  if (templateId==-1) {%>
...
-->			  <%if (templateId==-1) {%>
			  ...
			  <%}else{%>
				&nbsp;
				<input name="notuploadfile" type="button" class="singleboarder" value="<%=action%>(不上传文件)" onClick="return SubmitWithoutFile()">
			  <%}%>
      <input name="cmdcancel" type="button" class="singleboarder" onClick="ClearAll()" value=" 清 空 ">
&nbsp;&nbsp;
<!--<input name="remsg2" type="button" class="singleboarder" onClick="recorderDiv.style.display=''" value="录制语音">-->
<!--&nbsp;
...
&nbsp;
	  ...
	  %>
<input name="remsg" type="button" class="singleboarder" onClick='window.open("<%=viewPage%>?id=<%=id%>")' value=" 预 览 "></td>
              <%}%>
            </tr>
        </table>
    </form>
