...
<TABLE width="98%" BORDER=0 align="center" CELLPADDING=0 CELLSPACING=0 class="tableframe">
  <TR bgcolor="#FFFFFF">
    <TD width="489" class="right-title">&nbsp;通知内容</TD>
    <TD width="89" class="right-title"><a href="doc_list_notice.jsp?dir_code=notice" class="title_white">全部通知</a></TD>
  </TR>
...
String sql2 = "select id from sms_record where orgAddr=" + StrUtil.sqlstr(doc.getOrgAddr());
%>
回复的短信：
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#F0EFEE">
  <tr>
    <td width="22%" bgcolor="#FFFFFF"><strong>用户 </strong></td>
    <td width="22%" bgcolor="#FFFFFF"><strong>手机号 </strong></td>
    <td width="31%" bgcolor="#FFFFFF"><strong>收到信息 </strong></td>
    <td width="25%" bgcolor="#FFFFFF"><strong>回复时间</strong></td>
  </tr>
<%
java.util.Iterator sir = ssrd.list(sql2).iterator();
// 获得收到的信息
SMSReceiveRecordDb srrd = new SMSReceiveRecordDb();
java.util.Vector v = srrd.listReceived(doc);
int reCount = v.size();
UserDb ud2 = new UserDb();
int k = 0;
while (sir.hasNext()) {
	k ++;
	ssrd = (SMSSendRecordDb)sir.next();
	UserDb ud = ud2.getUserDbByMobile(ssrd.getSendMobile());

	// 检查该用户是否已回复
	boolean isReplied = false;
	for (int i=0; i<reCount; i++) {
		srrd = (SMSReceiveRecordDb)v.elementAt(i);
		if (srrd.getString("orgAddr").equals(ssrd.getSendMobile())) {
			isReplied = true;
			break;
		}
	}
  ...
  <tr>
	...
	if (ud!=null) {
		if (isReplied)
			...
			out.print("<font color=blue>" + ud.getRealName() + "</font>");
	}
	%>	</td>
    <td bgcolor="#FFFFFF"><%=ssrd.getSendMobile()%></td>
    <td bgcolor="#FFFFFF">
	<%
	if (isReplied)
		out.print(srrd.getString("msgtext"));
	%>	</td>
    <td bgcolor="#FFFFFF">
	<%
	if (isReplied)
		out.print(DateUtil.format(srrd.getDate("receivedate"), "yy-MM-dd HH:mm"));
	%>	</td>
  </tr>
<%}%>  
</table> 
总人数：<%=k%>，回复人数：<%=reCount%>，未回复人数：
<%=(k-reCount)%>
<%}%>
		  </td>
      </tr>
