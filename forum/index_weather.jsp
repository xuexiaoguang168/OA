...
<HTML><HEAD><TITLE><%=Global.AppName%> - <%=Global.server%></TITLE>
...
<table width="98%" class="table_normal"  border="1" align="center" cellpadding="1" cellspacing="0" bordercolor="<%=skin.getTableBorderClr()%>">
  <tr>
...
%>
<%if (myface.equals("")) {%>
	<img src="images/face/face<%=StrUtil.getNullString(me.getRealPic())%>.gif" width=64 title="<%=me.getName()%>">
<%}else{%>
...
<%}%>		  
	...
	<%}%>		  
		  <a href="../usercenter.jsp"><%=me.getName()%></a>&nbsp;&nbsp;<a href="javascript:hopenWin('../message/message.jsp',320,260)">
		  <lt:Label res="res.label.forum.index" key="msgbox"/>          
		  </a> (<a href="javascript:openScript('messanger.asp?action=read&amp;id=1838126&amp;sender=动网先锋论坛',500,400)"><font class="redfont"><%=msgcount%></font></a>)</td>
	...
	<table width="98%" align="center" border="0">
      <tbody>
        <tr>
          ...
          <td><a href="../listmember.jsp"><lt:Label res="res.label.forum.index" key="user_new"/></a> [<a href="../userinfo.jsp?username=<%=StrUtil.UrlEncode(forum.getUserNew())%>" target="_blank"><b><%=forum.getUserNew()%></b></a>]</td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          &nbsp;&nbsp;
          <lt:Label res="res.label.forum.index" key="most_post_date"/><%=DateUtil.format(forum.getMaxDate(), "yyyy-MM-dd")%>&nbsp;&nbsp;<!--<a href="javascript:showWeather()"><strong><font color="#ff0000">天气预报</font></strong></a>-->
          &nbsp;<a href="javascript:showWeather()"><strong><font color="#ff0000">天气预报</font></strong></a></td>
        </tr>
      </tbody>
    </table></td>
  </tr>
  ...
  <TABLE width="98%" height="24" border=0 align=center cellPadding=0 cellSpacing=1>
	...
	if (vnotice.size()!=0) {
		...
		while (irnotice.hasNext()) {
			...
			<a href="showtopic.jsp?rootid=<%=md.getId()%>"> 
			<%
			...
			<td>
					<table width="100%" border="0" cellpadding="0" cellspacing="0" >
					  <tr>
						...
						『 <a href="listtopic.jsp?boardcode=<%=lf.getCode()%>">
					  ...
					  <%}%>
						...
						<tr>
						  ...
						  <a title="<%=mdb.getTitle()%>" href="showtopic.jsp?rootid=<%=mdb.getId()%>"><%=StrUtil.toHtml(StrUtil.getLeft(mdb.getTitle(), 60))%></a></td>
						</tr>
						<tr>
						  <td>
						  <%if (md.isLoaded()) {%>
					  ...
					  <tr>
						...
						<%if (chcount>0) {%>
							...
							%>
								<a href="listtopic.jsp?boardcode=<%=StrUtil.UrlEncode(chlf.getCode())%>"><%=chlf.getName()%>&nbsp;</a>
							<%}%>
						  </td></tr></table>
						<%}%>
						</td>
	...
	%>
        <tr>
          ...
          <td><table width="100%" border="0" cellpadding="0" cellspacing="0" >
              <tr>
                <td width="47%" height="23"> 『 <a href="listtopic.jsp?boardcode=<%=lf.getCode()%>">
                  <%if (lf.getColor().equals("")) {%>
                  <%=lf.getName()%>
                  <%}else{%>
                  <font color="<%=lf.getColor()%>"><%=lf.getName()%></font>
...
<%@ include file="miniplugin/weather/index_inc.jsp"%>
