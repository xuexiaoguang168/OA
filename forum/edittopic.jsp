...
<form name=frmAnnounce method="post" action="edittopicdo.jsp" enctype="MULTIPART/FORM-DATA" onSubmit="return frmAnnounce_onsubmit()">
    <TBODY>
      <tr> 
      <td colspan="2" background="<%=skinPath%>/images/bg1.gif" class="td_title"> <lt:Label res="res.label.forum.edittopic" key="edittopic"/>&nbsp;<a href="showtopic.jsp?rootid=<%=Topic.getRootid()%>"><%=StrUtil.toHtml(Topic.getTitle())%></a></td>
      </tr>
    </TBODY>
    <TBODY>
      <tr>
...
<lt:Label res="res.label.forum.addtopic" key="file"/>&nbsp;<input type="file" name="filename" size=10>
      ...
      <tr>
        <td><iframe src="iframe_emotequick.jsp" height="35" name="emot" width="75%" marginwidth="0" marginheight="0" frameborder="0" scrolling="yes"></iframe>
        </td>
      </tr>
      <tr>
        <td><%
          ...
          <table width="98%"  border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="96%">&nbsp;
                ...
                &nbsp;<a href="javascript:delAttach('<%=am.getId()%>', '<%=Topic.getId()%>')"><lt:Label res="res.label.forum.addtopic" key="del_attach"/></a>&nbsp;&nbsp;<a target=_blank href="<%=am.getVisualPath()%>/<%=am.getDiskName()%>"><lt:Label res="res.label.forum.edittopic" key="view"/></a>&nbsp;&nbsp;</td>
            </tr>
          </table>
          <%}
			  }
