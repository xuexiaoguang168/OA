...
<%
		...
		String errorMessage = "";
		String rightMessage = "";
		PropertiesUtil pu = new PropertiesUtil(application.getRealPath("/") + "WEB-INF" + File.separator + "log4j.properties");
		...
		String imagepath = cloudHome + "images" + File.separator + "myface" + File.separator;
		...
		try {
			if (cloudHome != null) {
                ...
                if (!propError) {
					...
					if (readable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + cloudHome + "   读权限正确<br>";
					}
					// 检查Log目录是否有读写权限						
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + filelog.getParentFile() + "   读写权限正确<br>";
					}
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + cachepath + "   读写权限正确<br>";
					}					
					// 检查images目录上传是否有读写权限					
					readable = imagefile.canRead();
					if (!readable) {
						propError = true;
						errorMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + imagepath +
									SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","can_not_read") + "<br>";
					}
					writable = imagefile.canWrite();
					if (!writable) {
						propError = true;
						errorMessage +=  SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + imagepath +
									SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","can_not_write") + "<br>";
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + imagepath + "   读写权限正确<br>";
					}						
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + FileUploadTmp + "   读写权限正确<br>";
					}															
					// 检查doc目录上传是否有读写权限					
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + doc + "   读写权限正确<br>";
					}						
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + forumupfiledir + "   读写权限正确<br>";
					}												
					// 检查upfile目录上传是否有读写权限					
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + upfiledir + "   读写权限正确<br>";
					}	
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + classesdir + "   读写权限正确<br>";
					}	
					// 检查bak目录上传是否有读写权限					
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + bakdir + "   读写权限正确<br>";
					}	
					...
					if (readable && writable) {
					    rightMessage += SkinUtil.LoadString(request,"res.label.forum.admin.dir_check_pvg","dir") + themedir + "   读写权限正确<br>";
					}						
                }
	    	}
        	else {
...
<TABLE class="frame_gray" cellSpacing=0 cellPadding=0 width="95%" align=center>
  <TBODY>
    ...
    <TR>
      <TD width="41%" height=200 valign="top" bgcolor="#FFFBFF"><br>
        <ul><%
      ...
      <TD width="59%" valign="top" bgcolor="#FFFBFF"><br>
	  <%
	  if(!errorMessage.equals("")) {
	  %>
