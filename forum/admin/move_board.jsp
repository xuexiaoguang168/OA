...
<%
if (!privilege.isUserPrivValid(request, "forum.admin")) {
...
if (op.equals("move")) {
	...
	try {
		MsgDb md = new MsgDb();
		int count = md.moveBoardMessages(fromCode, toCode);
...
cellSpacing=0 cellPadding=3 width="95%" align=center>
  ...
  <form name=form1 action="?op=move" method=post>
        <tr>
          <td width="5%" height="28"><lt:Label res="res.label.forum.admin.move_board" key="from"/></td>
          <td width="24%" height="22"><select name="fromCode" onChange="if(this.options[this.selectedIndex].value=='not'){alert(this.options[this.selectedIndex].text+' <lt:Label res="res.label.forum.admin.move_board" key="can_not_select"/>'); return false;}">
          ...
          <td width="59%"> <lt:Label res="res.label.forum.admin.move_board" key="move_to"/>
          ...
          <td width="12%"><input value="<lt:Label key="ok"/>" type="submit"></td>
        </tr>
        </form>
      </table></TD>
    </TR>
