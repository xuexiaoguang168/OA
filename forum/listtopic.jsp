    ...
    <TBODY>
      <TR> 
		...
		<A href="<%=addpage%>?boardcode=<%=boardcode%>&threadType=<%=threadType%>&privurl=<%=privurl%>"> 
          ...
          <IMG src="<%=skinPath%>/images/votenew_<%=SkinUtil.getLocale(request)%>.gif" border=0 width=99 height=25 alt="<lt:Label res="res.label.forum.listtopic" key="vote_btn"/>"></A>
		<%
		if (vplugin.size()>0) {
			Iterator irplugin = vplugin.iterator();
			while (irplugin.hasNext()) {
...
<%}%>&nbsp;<a href="<%=ForumPage.getListTopicPage(request, boardcode, 1)%>" title="<lt:Label res="res.label.forum.showtopic" key="tree_view"/>"><img border=0 src="images/treeview.gif"></a>&nbsp;<a href="listtopic.jsp?op=showelite&boardcode=<%=boardcode%>">
...
<table align="center" width="98%"><tr><td height="2px"></td></tr></table>
<%
...
%>
<td class="<%=className%>" onMouseOver="this.className='td_hover'" onMouseOut="this.className='<%=className%>'"><span><a href="<%=ForumPage.getListTopicPage(request, boardcode, 0, 1, ttd.getId())%>"><%=ttName%></a></span></td>
<%}
	if (threadType==ThreadTypeDb.THREAD_TYPE_NONE)
		...
		className = "";
%>
<td class="<%=className%>" onMouseOver="this.className='td_hover'" onMouseOut="this.className='<%=className%>'"><span><a href="<%=ForumPage.getListTopicPage(request, boardcode)%>"><lt:Label res="res.label.forum.listtopic" key="topic_all"/></a></span></td>
</tr></table>
</div>
</td></tr></table>
<%}%>
